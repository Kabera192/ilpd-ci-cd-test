...
public class SwaggerConfig {
    ...
    private Info apiInfo() {
        return new Info().title("Inventory Service API")
                .description("Inventory Service API Documentation - JWT Secured").version("1.0.0")
    ...
    private Server getServer() {
        if (PROFILE_MODE.equals("prod")) {
            return new Server().url("https://misapi.ilpd.ac.rw/api/v1/inventory/")
                    .description("Production server via Gateway");
        }
        return new Server().url("http://localhost:8082/api/v1/inventory/")
                .description("Development server via Gateway");

    }
    private Server optServer() {
        return new Server().url("http://10.0.139.42:8082/api/v1/inventory/")
                .description("Development server via Gateway");
    }
}